        ...
        <div class="animate-in opacity-0 lg:col-span-5">
            <div class="content-card">
                ...
                <div class="content-card-body space-y-3">
                    @if($order->isPending())
                        <form method="POST" action="{{ route('admin.orders.mark-paid', $order) }}" onsubmit="return confirm('Confirm payment received for {{ $order->order_no }}? This will issue the license and email the customer.');">
                            @csrf
                            <button type="submit" class="w-full rounded-lg bg-accent px-4 py-3 text-sm font-bold text-white transition-colors duration-300 hover:bg-accent-hover">Mark as Paid</button>
                        </form>
                        ...
                        <form method="POST" action="{{ route('admin.orders.cancel', $order) }}" onsubmit="return confirm('Cancel this order?');">
                            @csrf
                            <button type="submit" class="w-full rounded-lg border px-4 py-2.5 text-sm font-semibold text-muted transition-colors duration-300 hover:text-danger" style="border-color:var(--border);">Cancel Order</button>
                        </form>
                    ...
                    @if(in_array($order->status, ['paid', 'delivered'], true))
                        <form method="POST" action="{{ route('admin.orders.refund', $order) }}" onsubmit="return confirm('Mark {{ $order->order_no }} as refunded? Downloads will be blocked. Refund the money in the SSLCommerz panel / by hand first.');">
                            @csrf
                            <button type="submit" class="w-full rounded-lg border px-4 py-2.5 text-sm font-semibold transition-colors duration-300" style="border-color:rgba(239,68,68,0.4);color:var(--danger);">Mark as Refunded</button>
                        </form>
