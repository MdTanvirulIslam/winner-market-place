        ...
        <div class="mb-6 flex flex-wrap items-center justify-between gap-3">
            ...
            <div class="flex items-center gap-3">
                @if(in_array($order->status, ['paid', 'delivered', 'refunded'], true))
                    <a href="{{ route('account.orders.invoice', $order) }}" class="s-btn-ghost !px-4 !py-2 text-[13px]">Invoice</a>
                @endif
            ...
            @if($order->isPending())
                <div class="s-card p-6">
                    ...
                    <form method="POST" action="{{ route('payment.start', $order) }}">
                        @csrf
                        <button type="submit" class="s-btn w-full !py-3">
                    ...
                    <h5 class="mb-3 mt-6 border-t border-border pt-5 text-sm font-bold text-text">Or Pay Manually</h5>
            ...
            @elseif($order->isDelivered())
                <div class="s-card p-6">
                    ...
                    @if($order->license_key)
                        <div class="mb-3 rounded-lg bg-input px-4 py-3 font-mono text-[13px] font-bold text-accent-light">{{ $order->license_key }}</div>
                    @endif
                    ...
                    <a href="{{ route('account.downloads') }}" class="s-btn mt-4 !px-4 !py-2.5">
